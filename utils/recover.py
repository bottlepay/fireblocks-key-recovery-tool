...
from yubihsm.exceptions import YubiHsmAuthenticationError, YubiHsmConnectionError, YubiHsmDeviceError
...
class RecoveryErrorHSMAuthentication(Exception):
    pass

class RecoveryErrorHSMConnection(Exception):
    pass

class RecoveryErrorHSMKeyNotFound(Exception):
    pass

class RecoveryErrorHSMKeyInvalidAlgorithm(Exception):
    pass

class RecoveryErrorHSMAmbiguousMobileKey(Exception):
    pass
...
def restore_key_and_chaincode(zip_path, private_pem_path_or_hsm_id, passphrase, key_pass=None, mobile_key_pem_path_or_hsm_id = None, mobile_key_pass = None, yubihsm_uri = None, yubihsm_authkey = None):
    privkeys = {}
    chain_code = None
    players_data = defaultdict(dict)
    key_metadata_mapping = {}

    yubihsm_session = None

    # If yubihsm_uri is not defined then load encryption keys from the filesystem
    if yubihsm_uri is None:
        # Load the recovery private key
        with open(private_pem_path_or_hsm_id, 'r') as _file:
            key_pem = _file.read()
        ...
        key_decrypt_func = cipher.decrypt

        # Load the mobile key encryption private key
        if passphrase is None:
            with open(mobile_key_pem_path_or_hsm_id, 'r') as _file:
            ...
            mobile_key_decrypt_func = mobile_cipher.decrypt
    else:
        try:
            ...
            raise RecoveryErrorHSMConnection()

        # lookup the recovery private key within the HSM
        try:
            key = yubihsm_session.get_object(int(private_pem_path_or_hsm_id), OBJECT.ASYMMETRIC_KEY)
            key_info = key.get_info()

            # ensure it's the expected algorithm
            if key_info.algorithm != ALGORITHM.RSA_2048 and key_info.algorithm != ALGORITHM.RSA_3072 and key_info.algorithm != ALGORITHM.RSA_4096:
                raise RecoveryErrorHSMKeyInvalidAlgorithm()

            key_decrypt_func = lambda d: key.decrypt_oaep(d, hash=hashes.SHA1(), mgf_hash=hashes.SHA1())
        except YubiHsmDeviceError as ex:
            if ex.code == ERROR.OBJECT_NOT_FOUND:
                raise RecoveryErrorHSMKeyNotFound()


        if mobile_key_pem_path_or_hsm_id is not None:
            if passphrase is not None:
                raise RecoveryErrorHSMAmbiguousMobileKey()

            # lookup the auto-generated passphrase wrap key
            try:
                mobile_key = yubihsm_session.get_object(int(mobile_key_pem_path_or_hsm_id), OBJECT.ASYMMETRIC_KEY)
                mobile_key_info = mobile_key.get_info()
                ...
                if name.startswith("MOBILE"):
                    ...
                    try:
                        if (passphrase):
                            ...
                            try:
                                data = mobile_key_decrypt_func(bytes.fromhex(obj["encryptedKey"]))
                            except ValueError:
                    ...
                    if key_id:
                        data = key_decrypt_func(file.read())
