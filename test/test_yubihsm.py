...
# This suite is intended to be run on a freshly factory-reset HSM.
YUBIHSM_URI = 'http://localhost:12345'
YUBIHSM_AUTHKEY = 1
YUBIHSM_AUTHKEY_PASSWORD = 'password'

hsm = YubiHsm.connect(YUBIHSM_URI)
yubihsm_session = hsm.create_session_derived(YUBIHSM_AUTHKEY, YUBIHSM_AUTHKEY_PASSWORD)

def cleanup():
    for key in [101, 102]:
        key = yubihsm_session.get_object(key, OBJECT.ASYMMETRIC_KEY)
        try:
            key.delete()
        except YubiHsmDeviceError as err:
            if err.code != ERROR.OBJECT_NOT_FOUND:
                return

def import_key(id, path, key_pass=None):
    print('importing', path, 'to hsm...')
    with open(path, 'rb') as _file:
        key_pem = _file.read()

    key = serialization.load_pem_private_key(key_pem, password=key_pass)
    AsymmetricKey.put(yubihsm_session, id, 'path', 1, CAPABILITY.EXPORTABLE_UNDER_WRAP | CAPABILITY.DECRYPT_PKCS | CAPABILITY.DECRYPT_OAEP, key)

def setup():
    import_key(101, 'priv.pem')
    import_key(102, 'priv2.pem')

def test_recovery():
    privkeys, chaincode = recover.restore_key_and_chaincode("backup.zip", "101", "Thefireblocks1!", key_pass=YUBIHSM_AUTHKEY_PASSWORD, yubihsm_uri=YUBIHSM_URI, yubihsm_authkey=YUBIHSM_AUTHKEY)
...
def test_full_recovery():
    privkeys, chaincode = recover.restore_key_and_chaincode("backup_new.zip", "102", "Thefireblocks1!", key_pass=YUBIHSM_AUTHKEY_PASSWORD, yubihsm_uri=YUBIHSM_URI, yubihsm_authkey=YUBIHSM_AUTHKEY)
...
def test_recovery_old_format():
    privkeys, chaincode = recover.restore_key_and_chaincode("backup_old_format.zip", "101", "Thefireblocks1!", key_pass=YUBIHSM_AUTHKEY_PASSWORD, yubihsm_uri=YUBIHSM_URI, yubihsm_authkey=YUBIHSM_AUTHKEY)
    assert(privkeys['MPC_ECDSA_SECP256K1'] == 0x473d1820ca4bf7cf6b018a8520b1ec0849cb99bce4fff45c5598723f67b3bd52)
    pub = recover.get_public_key("MPC_ECDSA_SECP256K1", privkeys['MPC_ECDSA_SECP256K1'])
    assert(pub == "021d84f3b6d7c6888f81c7cc381b658d85319f27e1ea9c93dff128667fb4b82ba0")
    assert(recover.encode_extended_key('MPC_ECDSA_SECP256K1', privkeys['MPC_ECDSA_SECP256K1'], chaincode, False) == "xprv9s21ZrQH143K24Mfq5zL5MhWK9hUhhGbd45hLXo2Pq2oqzMMo63oStZzF9aunJDs4SsrmoxycAo6xxBTHawSz5sYxEy8TpCkv66Sci373DJ")
...
def test_cmp_recovery():
    privkeys, chaincode = recover.restore_key_and_chaincode("backup_cmp.zip", "101", "Fireblocks1!", key_pass=YUBIHSM_AUTHKEY_PASSWORD, yubihsm_uri=YUBIHSM_URI, yubihsm_authkey=YUBIHSM_AUTHKEY)
